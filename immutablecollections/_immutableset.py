...
def immutableset(
    ...
    *,
    ...
    order_key: Optional[Callable[[T], Any]] = None,
) -> "ImmutableSet[T]":
    """
    ...
    If *check_top_matches* is specified, each element added to this set will be checked to be an
    instance of that type.

    If *order_key* is present, the order of the resulting set will be sorted by that key function
    rather than in the usual insertion order.
    ...
    if iteration_order:
        if len(iteration_order) == 1:
            ...
            if order_key is not None:
                iteration_order = sorted(iteration_order, key=order_key)
...
class ImmutableSet(  # pylint: disable=duplicate-bases
    ...
    ImmutableSets should be created via the module-level constructor `immutableset()`, not by
    directly instantiating one of the sub-classes.
    ...
    def __init__(self, top_level_type: Optional[Type]) -> None:
    ...
    def of(
        ...
        if isinstance(seq, ImmutableSet):
            ...
            return immutableset(
                ...
                disable_order_check=not require_ordered_input,
            )
    ...
    def union(
        ...
        """
        return immutableset(
            list(self) + list(other), check_top_type_matches=check_top_type_matches
        )
    ...
    def intersection(self, other: Iterable[Any]) -> "ImmutableSet[T]":
        ...
        """
        return immutableset(
            (x for x in self if x in other), check_top_type_matches=self._top_level_type
        )
...
class _FrozenSetBackedImmutableSet(ImmutableSet[T]):
    ...
    __slots__ = "_set", "_iteration_order"
